...
ref=$EVAL_FOLDER/$ORIG_DEV_PREFIX.$TARGET_LANG_SUFFIX

echo "Computing BLEU for dev set"
cat $hyp | sed -r 's/\@\@ //g' | $moses_scripts/recaser/detruecase.perl | sacrebleu $ref -l $SOURCE_LANG_SUFFIX-$TARGET_LANG_SUFFIX

hyp=$INFER_OUTPUT_FOLDER/$CORPUS.$RUN.$TEST_PREFIX.hyp.$TARGET_LANG_SUFFIX
ref=$EVAL_FOLDER/$ORIG_TEST_PREFIX.$TARGET_LANG_SUFFIX
