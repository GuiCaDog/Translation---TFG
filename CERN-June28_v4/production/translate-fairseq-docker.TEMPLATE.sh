...
do 
    ...
    cat $in_f | $moses_scripts/tokenizer/normalize-punctuation.perl -l $src | $moses_scripts/tokenizer/tokenizer.perl -a -no-escape -l $src | $moses_scripts/recaser/truecase.perl -model $SYSTEM_DIRECTORY/truecase-model.$src | python3 $SUBWORD_FOLDER/apply_bpe.py -c $BPE_FOLDER/bpe.codes --vocabulary $BPE_FOLDER/bpe.vocab.$src --vocabulary-threshold 50 > $TMP/$filename.prepro
